...
void kill_screen(const char* lcd_msg) {
  ...
  inline void line_to_z(const float &z) {
	...
	int item_number;
	int camilles_test_var;
	char* filename;
	
	void print_by_number(uint16_t num) {
		filename = card.get_name_by_index(num);
		card.openAndPrintFile(filename);
	}
	
	
	void auto_print_code_enter() {
		...
		MENU_ITEM_EDIT(int3, MSG_AUTO_MSG, &camilles_test_var, 0, 100);
		MENU_ITEM(function, MSG_PRINT, print_by_number(camilles_test_var));
		END_MENU();
	}
	...
	#if defined(USER_DESC_1) 
		void lcd_camille_test() {lcd_goto_screen(auto_print_code_enter);}
    ...
    void _lcd_user_menu() {
      ...
      MENU_BACK(MSG_MAIN);
      #if defined(USER_DESC_1)
        MENU_ITEM(function, USER_DESC_1, lcd_camille_test);
      #endif
      #if defined(USER_DESC_2) && defined(USER_GCODE_2)
